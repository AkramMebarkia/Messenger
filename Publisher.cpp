...
int ACE_TMAIN(int argc, ACE_TCHAR* argv[])
{
  try {
    ...
    writer_qos.ownership.kind = DDS::SHARED_OWNERSHIP_QOS;
    ...
    message.subject = "Latency Test";
    message.subject_id = 1;
    message.count = 0;

    // Set payload size (can be modified: 256, 1024, 5120, or 10240 bytes)
    int payload_size = 256; // 256, 1024, 4096, 16384
    int number_of_messages = 100; // Number of messages to send

    for (int i = 0; i < number_of_messages; ++i) {
        // Set payload for each message
        message.payload.length(payload_size);
        for (CORBA::ULong j = 0; j < payload_size; ++j) {
            message.payload[j] = (CORBA::Octet)(j % 256);
        }
